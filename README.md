...
### Adding New Features
1. Update data models in `internal/models/`
2. Implement service logic in appropriate package
...
**Issue**: Server fails to start with "JWT_SECRET environment variable is required"
**Solution**: Set the `JWT_SECRET` environment variable before starting the server.

**Issue**: "Failed to load data files"
**Solution**: Ensure `customers.json` and `transactions.json` exist in the `data/` directory.
...
**Solution**: Check that the password is hashed correctly and the user exists in the system.
